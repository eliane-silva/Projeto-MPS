...
public class MemoryUserDAO implements UserDAO {
    private static final AppLogger log =
            AppLoggerFactory.getLogger(MemoryUserDAO.class);
    ...
    public User salvar(User usuario) throws RepositoryException {
        try {
            if (usuario == null) throw new RepositoryException("Usuário não pode ser nulo");
    ...
    public User buscarPorId(int id) throws RepositoryException {
        try {
            ...
            log.error("Erro ao buscar usuário por ID: {}", e, id);
    ...
    public User buscarPorLogin(String login) throws RepositoryException {
        try {
            ...
            return users.stream().filter(u -> key.equals(u.getLogin())).findFirst().orElse(null);
        } catch (Exception e) {
    ...
    public boolean excluir(int id) throws RepositoryException {
        try {
            ...
            if (removed) log.info("Usuário excluído: {}", id);
            return removed;
        } catch (Exception e) {
            log.error("Erro ao excluir usuário: {}", e, id);
